...
var parseSetEnumTypeDef = function (type, prefixLen) {
  ...
  return type
    .substr(prefixLen, type.length - prefixLen - 1)
    .split(",")
    .map(function (opt) {
      return opt[0] === '"' || opt[0] === "'"
        ? opt.substr(1, opt.length - 2)
        : opt;
    });
};
...
var readInt24BE = function (buf, offset, noAssert) {
  return (
    (buf.readInt8(offset, noAssert) << 16) +
...
var readIntBE = function (buf, offset, length, noAssert) {
  switch (length) {
    ...
    case 3:
      return readInt24BE(buf, offset, noAssert);
    case 4:
...
var parseNewDecimal = function (parser, column) {
  ...
  var size =
    uncompIntegral * 4 +
    compressedBytes[compIntegral] +
    uncompFractional * 4 +
    compressedBytes[compFractional];
    ...
    case MysqlTypes.STRING:
      ...
      if (defPrefix === "binary") {
        ...
        if (column.charset !== null) {
          ...
          column.charset =
            column.charset === "utf8mb4" || "utf8mb3" ? "utf8" : column.charset;
